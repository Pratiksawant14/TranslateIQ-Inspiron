    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="font-display text-[28px] font-bold text-[var(--color-text-primary)]">Projects</h1>
            <p className="text-[var(--color-text-muted)] mt-1">Manage your translation projects</p>
          </div>
        </div>
    ...
    return (
      <div className="space-y-6">
        ...
        <Card className="p-8 text-center">
          ...
          <h3 className="text-lg font-semibold text-[var(--color-text-primary)] mb-2">Failed to load projects</h3>
          <p className="text-[var(--color-text-muted)]">Could not connect to the server. Please check if the backend is running.</p>
        </Card>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      {isEmpty ? (
        <Card className="p-12 flex flex-col items-center justify-center text-center">
          ...
          <h3 className="text-xl font-display font-semibold text-[var(--color-text-primary)] mb-2">No projects yet</h3>
          ...
          {projects.map((project) => (
            <Card key={project.id} className="p-6 flex flex-col group" onClick={() => navigate(`/projects/${project.id}`)}>
              <div className="flex items-start justify-between mb-3">
                <h3 className="font-display font-bold text-[var(--color-text-primary)] text-lg leading-tight group-hover:text-blue-400 transition-colors">
                  {project.name}
                </h3>
              ...
              {!project.description && (
                <p className="text-sm text-[var(--color-text-secondary)] italic mb-4 flex-1">No description</p>
              )}

              <div className="flex items-center gap-4 text-xs text-[var(--color-text-secondary)] mb-4 pt-3 border-t border-[var(--color-border)]/30">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <textarea
                ...
                className="w-full px-3 py-2 rounded-lg bg-[var(--color-bg-surface)] border border-[var(--color-border)] text-[var(--color-text-primary)] text-sm placeholder:text-[var(--color-text-secondary)]
